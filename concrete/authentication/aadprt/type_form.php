...
<div class="alert alert-info">
    ...
    <p><?php echo t('<a href="%s" target="_blank">Click here</a> to obtain your access keys.', 'https://apps.aadprt.com/'); ?></p>
    <p><?php echo t('Check the box labeled "Allow this application to be used to Sign in with Twitter".'); ?></p>
    <p><?php echo t('Set the "Callback URL" to:%s.', ' <code>'.\URL::to('/ccm/system/authentication/oauth2/aadprt/callback').'</code>'); ?></p>
</div>
...
<div class='form-group'>
    <div class="input-group">
        <label type="checkbox">
            <input type="checkbox" name="registration_enabled" value="1" <?php echo \Config::get('auth.aadprt.registration.enabled', false) ? 'checked' : '' ?>>
    ...
    <select name="registration_group" class="form-control">
        ...
        foreach ($groups as $group) {
            ...
            <option value="<?php echo $group->getGroupID() ?>" <?php echo intval($group->getGroupID(), 10) === intval(
                \Config::get('auth.aadprt.registration.group', false),
                10) ? 'selected' : '' ?>>

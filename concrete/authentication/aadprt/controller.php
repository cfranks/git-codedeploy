<?php
namespace Concrete\Authentication\Aadprt;
...
use OAuth\OAuth1\Service\Aadprt;
...
class Controller extends GenericOauth1aTypeController
{
    ...
    {
        return \Config::get('auth.aadprt.registration.group');
    }

    /**
     * Aadprt doesn't give us the users email.
     *
     * @return bool
     */
    public function supportsRegistration()
    {
        return \Config::get('auth.aadprt.registration.enabled', false);
    }
    ...
    {
        if (!$this->service) {
            $this->service = \Core::make('authentication/aadprt');
        }
        return $this->service;
    }
    ...
    {
        \Config::save('auth.aadprt.appid', $args['apikey']);
        \Config::save('auth.aadprt.secret', $args['apisecret']);
        \Config::save('auth.aadprt.registration.enabled', !!$args['registration_enabled']);
        \Config::save('auth.aadprt.registration.group', intval($args['registration_group'], 10));
    }

    public function edit()
    {
        $this->set('form', \Loader::helper('form'));
        $this->set('apikey', \Config::get('auth.aadprt.appid', ''));
        $this->set('apisecret', \Config::get('auth.aadprt.secret', ''));
    ...
    /**
     * We override this method because aadprt doesn't give us the email, we have to have the user input it before we can create a user.
     * @return null|\User
     * @throws Exception
     */
    protected function attemptAuthentication()
    {
        ...
        if ($this->supportsRegistration()) {
            ...
            $response = \Redirect::to('/login/callback/aadprt/handle_register/', id(new Token)->generate('aadprt_register'));
            $response->send();
            exit;
        }
    ...
    public function handle_register($token=null) {
        ...
        if (!$token_helper->validate('aadprt_register', $token) && !$token_helper->validate('aadprt_register') ||
            !$this->token) {
            $this->redirect('/login/');
            exit;
        }

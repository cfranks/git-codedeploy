...
if (count($navItems) > 0) {
    ...
    foreach ($navItems as $ni) {
        ...
        if ($ni->hasSubmenu) {
	    if ($ni->cObj->getAttribute('mega_menu_page') == 1) {
            $page_object = $ni->cObj;
            $blurb = $page_object->getAttribute('mega_menu_blurb');
            $link = $page_object->getAttribute('mega_menu_link');
	    if ($link > 0) {
	        $link = Page::getByID($link);
		if (is_object($link)) { 
			$link = $link->getCollectionPath();
		} else {
		    $link = '/';
		}
	    } else {
            ...
            $link_text = $page_object->getAttribute('mega_menu_link_text');
            try {
                if ($page_object->getAttribute('mega_menu_image')) {
                    $f = File::getByID($page_object->getCollectionAttributeValue('mega_menu_image'));
                    $image = Core::make('html/image', array($f));
                    $imagePath = $image->getTag()->src;
                    $image_html = '<img src="' . $imagePath . '" alt="' . $ni->name . '">';
                }
                else {
                    $image_html = '';
                }
            ...
            if ($link && $link_text) {
                $link = '<a href="' . $link . '" class="btn-theme">' . $link_text . '</a>';
            }
            else {
                $link = '';
            }
            echo '<div class="mega-menu"><div class="menu-content">' . $image_html . ' ' . $blurb . ' ' . $link . '</div><ul>';
        } else {
            echo '<div class="mega-menu"><ul>'; //opens a dropdown sub-menu
	}
        } else {

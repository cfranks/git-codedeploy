...
if (count($navItems) > 0) {
	...
	if (in_array($ni->cID, $exclude)) {
	    ...
	    if ($ni->cObj->getAttribute('mega_menu_page') == 1) {
	    	echo '<div class="mega-menu"><ul>';
	    	$level = $ni->level;
            	$children = $ni->cObj->getCollectionChildren();
		foreach ($children as $child) {
                	$exclude[] = $child->getCollectionID();
                	$p = $child;
			$name = $p->getCollectionName();
                	if ($p->getAttribute('exclude_nav') != 1) {
				$options[(string)$p->getAttribute('page_category_type')][] = '<li><a href="' . Core::make('helper/navigation')->getLinkToCollection($p) . '">' . $name . '</a></li>';
			}
		}
		foreach($options as $key => $opt) {
			echo '<li><span class="nav-title">'.$key.'</span>
              <ul>
                ' . implode("", $opt) . '
              </ul>
            </li>';

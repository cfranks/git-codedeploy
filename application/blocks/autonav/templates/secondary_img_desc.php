...
echo '<div class="page-list">'; //opens the top-level menu

foreach ($navItems as $ni) {
   
   echo '<div class="list-item"><a href="' . $ni->url . '" target="' . $ni->target . '" class="' . $ni->classes . '">';
   $img = $ni->cObj->getAttribute('thumbnail');
   if ($img) {
          if (is_object($img)) {
	  ...
	  $thumb = $ih->getThumbnail($img, 600, 300, false);
	  
	  echo '<div class="fun-img"><img src="' . $thumb->src . '" width="' . $thumb->width . '" height="' . $thumb->height . '" alt="' . $altText . '" /></div>';
   
   } else {
	  echo '<div class="fun-img"><img src="' . $view->getThemePath() . '/img/thumbnail-default.jpg" alt="AADPRT Logo"/></div>';  
	 }
   echo '<div class="feature"><h3>' ; echo $ni->cObj->getCollectionName(); echo '</h3>' ;
   echo '<p>' ; echo $ni->cObj->getCollectionDescription();echo '</p>' ;
   echo '<p class="btn-text">Learn More</p>' ;
   echo '</a></div></div>' ;
}
echo '</div>' ;
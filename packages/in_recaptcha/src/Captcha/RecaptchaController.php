...
namespace Concrete\Package\InRecaptcha\Src\Captcha;
...
class RecaptchaController extends CaptchaController
{
    public function saveOptions($data)
    {
        ...
        $config->save('captcha.type', $data['type']);
    }
    ...
    function showInvisible()
    {
        ...
        $assetUrl = 'https://www.google.com/recaptcha/api.js';

        $assetList->register('javascript', 'in_recaptcha_api', $assetUrl, array('local' => false));

        $assetList->register('javascript', 'in_recaptcha_render', 'assets/js/render.js', array(), 'in_recaptcha');

        $assetList->registerGroup(
            'in_recaptcha',
            array(
                array('javascript', 'in_recaptcha_render'),
                array('javascript', 'in_recaptcha_api'),
            )
        );

        $responseAssets = ResponseAssetGroup::get();
        $responseAssets->requireAsset('in_recaptcha');

        echo '<div id="recaptcha" class="g-recaptcha" data-badge="inline" data-
    ...
    function showV2()
    {
        ...
        $assetList->register('javascript', 'in_recaptcha_render', 'assets/js/render_v2.js', array(), 'in_recaptcha');
    ...
    {
        $pkg = Package::getByHandle('in_recaptcha');

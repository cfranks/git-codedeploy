...
class SafeSave 
{
    ...
    function setLogin($username = 'SOTDW-Test', $password = 'TestSociety12') 
    {
        
        if(!empty(Config::get('mass_enrollment::integration'))) {
            if(Config::get('mass_enrollment::integration.flagTestMode') == 1) {
               $username = Config::get('mass_enrollment::integration.testsafeUsername');
               $password = Config::get('mass_enrollment::integration.testsafePassword');
            } else {
	    	$username = Config::get('mass_enrollment::integration.safeUsername');
            	$password = Config::get('mass_enrollment::integration.safePassword');
	    }
  ...
  // Transaction Functions
    ...
    function doTokenPayment($amount, $payment_token)
    {
    	$query  = "";
        // Login Information
        $query .= "username=" . urlencode($this->login['username']) . "&";
    ...
    {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, "https://secure.safesavegateway.com/api/transact.php");

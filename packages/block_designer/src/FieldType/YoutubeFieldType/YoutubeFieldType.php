<?php  namespace Concrete\Package\BlockDesigner\Src\FieldType\YoutubeFieldType;
...
class YoutubeFieldType extends FieldType
{
    ...
    public function getViewContents()
    {
        ...
        $iframeData = array(
            ...
            'src'             => $baseURL . '<?php  echo $' . $this->data['slug'] . '; ?>' . (!empty($getData) ? '?' . htmlspecialchars(http_build_query($getData)) . (isset($getData['loop']) ? '&amp;playlist=<?php  echo $' . $this->data['slug'] . '; ?>' : null) : null),
            'frameborder'     => '0',
        ...
        return '<?php  if (isset($' . $this->data['slug'] . ') && trim($' . $this->data['slug'] . ') != ""){ ?>' . $this->data['prefix'] . '<?php  $c = Page::getCurrentPage();
    if ($c->isEditMode()) {
    ...
    {
        return '$' . $this->data['slug'] . '_code = null;
        ...
        if (!empty($matches) && isset($matches[1]) && trim($matches[1]) != "") {
            $' . $this->data['slug'] . '_code = $matches[1];
        }
        $this->set("' . $this->data['slug'] . '", $' . $this->data['slug'] . '_code);';
    }
    ...
    public function getValidateFunctionContents()
    {
        return 'if (in_array("' . $this->data['slug'] . '",$this->btFieldsRequired) && trim($args["' . $this->data['slug'] . '"]) == ""){
            ...
            if ($args["' . $this->data['slug'] . '"] != "" && (!isset($matches[1]) || trim($matches[1]) == "")) {
                $e->add(t("The %s field contains an invalid YouTube URL.", "' . htmlspecialchars($this->data['label']) . '"));
            }
        }';
    }

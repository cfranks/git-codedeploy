...
if (count($cArray) > 0) {
    ...
    <div class="ccm-page-list">
        ...
        for ($i = 0; $i < count($cArray); $i++) {
            ...
            ?>
            <div class="content-sbBlog-wrap" style="overflow: auto;">
        ...
        <?php echo Core::make('helper/date')->formatCustom(t('M d, Y'), strtotime($blogDate)); ?>
                ...
                <br class="clearfloat">
                <div style="clear: left; float: left;">  <a class="readmore" href="<?php echo $url ?>"><?php echo t('Read More') ?></a></div>
                <!--                <div id="tags" style="clear: left; float: left;">
                                <b><?php echo t('Tags') ?> : </b>
                <?php
                if (!empty($tag_list)) {
                    $x = 0;
        ...
        if ($subscribe && $u->isLoggedIn()) {
            ...
            <div id="subscribe_to_blog" class="ccm-ui">
                <a href="<?php echo $subscribe_link; ?>?blog=<?php echo $cParentID; ?>&user=<?php echo $u->getUserID(); ?>" onClick="javascript:;" class="subscribe_to_blog btn btn-default btn-small" data-status="<?php if ($subscribed_status) {
            echo 'unsubscribe';
        } else {
            echo 'subscribed';
        } ?>"> <?php if ($subscribed_status) {
            echo t('Unsubscribe from this Blog');
        } else {
            echo t('Subscribe to this Blog');
        } ?> </a>
            </div>
            <?php
        }

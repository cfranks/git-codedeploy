				...
				<?php 
					if ($form) { 
						...
						$bt->controller->view_type = $view_type;
					  	$bt->render('/templates/dashboard/view');
					} 
				?>
				...
				<?php } else { ?>				
				    ...
				    <script>
						...
						function downloadImage(image) {
							var source = $(image).find('img');
							var title = $(image).data('title');
							if (title.length <= 0) title = 'download.png';
							var canvas = document.createElement('canvas');
							canvas.width = source.width() * 2;
							canvas.height = source.height() * 2;
							context = canvas.getContext("2d");														
							context.drawImage(source[0], 0, 0, source.width() * 2, source.height() * 2);							

...
<?php } else { ?>
    ...
    <?php 
        if ($task == 'delete_all') { 
            $action = \URL::to('/formidable/dialog/dashboard/results/delete_all/submit');
            $button = t('Delete all');
        ?> 
        <div class="alert alert-warning">
            <?php echo t('Are you sure you want to delete ALL results of this form?'); ?>
        </div>
    <?php } ?>

...
<?php } else { ?>
	...
	<?php } elseif ($task == 'action') { ?>
		...
		<div class="dependency_action" id="action_<?php echo $action_rule; ?>" style="margin-top: 5px;">
			<div class="action_buttons" style="width:16%;float:right;">
				<?php if ($action_rule == 0 || $action_rule == 100) { ?>
					<a href="javascript:ccmFormidableAddDependencyAction(<?php echo $current_element->getElementID(); ?>, <?php echo $rule; ?>);" class="btn btn-success option_button" title="<?php echo t('Add an action to this rule'); ?>">+</a>
				<?php } ?>
				<a href="javascript:ccmFormidableDeleteDependencyAction(<?php echo $rule; ?>, <?php echo $action_rule; ?>);" class="btn btn-danger option_button" title="<?php echo t('Delete this action'); ?>" <?php if ($action_rule != 0 && $action_rule != 100) { ?>style="margin-left:55px;"<?php } ?>>-</a>
			</div>
	...
	<?php } elseif ($task == 'element') { ?>
		...
		<div class="dependency_element" id="element_<?php echo $element_rule; ?>" style="margin-top: 5px;">
			...
			?>
			<div class="action_buttons" style="width:16%;float:right;">
				<?php if ($element_rule == 0 || $element_rule == 100) { ?>
					<a href="javascript:ccmFormidableAddDependencyElement(<?php echo $current_element->getElementID(); ?>, <?php echo $rule; ?>);" class="btn btn-success option_button" title="<?php echo t('Add an element to this rule'); ?>">+</a>
				<?php } ?>
				<a href="javascript:ccmFormidableDeleteDependencyElement(<?php echo $rule; ?>, <?php echo $element_rule; ?>);" class="btn btn-danger option_button" title="<?php echo t('Delete this element'); ?>" <?php if ($element_rule != 0 && $element_rule != 100) { ?>style="margin-left:55px;"<?php } ?>>-</a>
			</div>

...
class Slider extends Element {
	...
	public function generateInput() {				
		...
		if (empty($val)) $val = $range?'['.$attribs['data-slider-min'].','.$attribs['data-slider-max'].']':$attribs['data-slider-min'];
		...
		$this->addJavascript("if ($.fn.bootstrapSlider) { $('#".$this->getHandle()."').bootstrapSlider({range:".($range?'true':'false').$options."});}");
